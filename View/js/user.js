...
function changeFood(foodId, delta, btn) {
    ...
    try {
        // 🔥 FIX encode %7B%22
        const decoded = decodeURIComponent(btn.dataset.food);
        food = JSON.parse(decoded);
    } catch (e) {
        console.error("Lỗi parse food:", btn.dataset.food);
        return;
    }

    if (!state.foods[foodId]) {
        state.foods[foodId] = { data: food, qty: 0 };
    }
    ...
    state.foods[foodId].qty = Math.max(
        0,
        state.foods[foodId].qty + delta
    );

    const el = document.getElementById('qty-' + foodId);
    if (el) el.textContent = state.foods[foodId].qty;
